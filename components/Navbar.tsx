...
import { User, LogIn, Plus, Crown, Settings, CircleHelp, MessageCircle, FileText } from 'lucide-react';
...
export default function Navbar() {
  ...
  const menuItems = [
    ...
    { name: 'New Trip', icon: <Plus size={18} /> },
    { name: 'Manage Subscription', icon: <Crown size={18} /> },
    { name: 'Settings', icon: <Settings size={18} /> },
    { name: 'About', icon: <CircleHelp size={18} /> },
                  ...
                  <div className="py-2">
                    {menuItems.map((item, index) => (
                      <div key={index}>
                        <button
                          type="button"
                          className="flex w-full items-center gap-3 px-5 py-3 text-sm text-gray-700 hover:bg-blue-50 transition"
                        >
