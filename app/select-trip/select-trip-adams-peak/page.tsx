...
export default function Home() {
  ...
  const handleSend = (text?: string) => {
    ...
    if (userText.includes('hi')) {
      response = 'Hi 👋 PearlGo here! Ready for your Sri Lanka adventure!';
    ...
    } else if (userText.includes('adams') || userText.includes('peak')) {
      response = '⛰️ Adam’s Peak (Sri Pada) — sacred mountain with legendary sunrise views!';
    } else if (userText.includes('sunrise')) {
      response = '🌅 Best sunrise hike starts around 2 AM to reach summit by dawn!';
    } else if (userText.includes('difficult')) {
      response = '🥾 Moderate difficulty — ~5,000 steps but very rewarding!';
    } else if (userText.includes('add cities')) {
      response = '🌆 Added Ella + Nuwara Eliya + Hatton!';
    } else {
      response = '🤖 I can help with budget, hiking routes & itinerary!';
    }
        ...
        <div className="flex-1 p-8 space-y-8 overflow-y-auto">
          ...
          >
            <div className="relative w-52 h-36 rounded-2xl overflow-hidden">
              <Image src="/adams-peak.webp" alt="Adam's Peak" fill className="object-cover" />
            </div>

            <div className="flex-1">
              <h1 className="text-3xl font-bold text-gray-800">Adam’s Peak (Sri Pada) ⛰️</h1>

              <div className="flex gap-5 text-sm text-gray-500 mt-3">
                <span className="flex items-center gap-1">
                  <Calendar size={14} /> 1–2 Days
                </span>
                <span className="flex items-center gap-1">
                  <MapPin size={14} /> Central Highlands
                </span>
                <span className="flex items-center gap-1">
                  <Plane size={14} /> Hiking Route
                </span>
              </div>
            </div>
          ...
          >
            <h2 className="text-2xl font-bold text-gray-800">⛰️ Adam’s Peak</h2>

            <p className="text-gray-600 mt-2 text-sm">
              A sacred mountain in Sri Lanka famous for the Sri Pada footprint and breathtaking sunrise views.
            </p>

            <iframe
              src="https://www.google.com/maps?q=Adams%20Peak%20Sri%20Lanka&output=embed"
            ...
            <div className="mt-6 relative overflow-hidden rounded-3xl p-6 bg-gradient-to-br from-indigo-50 via-white to-blue-50 border border-white/50 shadow-lg">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800">Why climb Adam’s Peak?</h3>
                  <p className="text-xs text-gray-500">Sacred Pilgrimage & Sunrise Experience</p>
                </div>
              </div>

              <p className="text-sm text-gray-700 leading-relaxed">
                <span className="font-semibold text-indigo-700">Adam’s Peak</span> is a multi-religious sacred site
                famous for its glowing sunrise shadow and spiritual journey to the summit 🌄
              </p>

              <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="flex items-start gap-2 bg-white/60 p-3 rounded-xl shadow-sm">
                  <span>🌄</span>
                  <p className="text-sm text-gray-700">World-famous sunrise view</p>
                </div>

                <div className="flex items-start gap-2 bg-white/60 p-3 rounded-xl shadow-sm">
                  <span>⛰️</span>
                  <p className="text-sm text-gray-700">Sacred Sri Pada footprint</p>
                </div>

                <div className="flex items-start gap-2 bg-white/60 p-3 rounded-xl shadow-sm">
                  <span>🥾</span>
                  <p className="text-sm text-gray-700">~5,000 step hiking trail</p>
                </div>

                <div className="flex items-start gap-2 bg-white/60 p-3 rounded-xl shadow-sm">
                  <span>🕊️</span>
                  <p className="text-sm text-gray-700">Spiritual pilgrimage experience</p>
                </div>
              </div>

              <div className="mt-5 flex items-center justify-between">
                <div className="text-xs text-gray-500">⏱ Recommended hike: Night start (2–3 AM)</div>
