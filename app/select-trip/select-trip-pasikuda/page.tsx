...
export default function Home() {
  ...
  const suggestions = [
    ...
    'Places near Pasikuda Beach',
    'Best time to visit Sri Lanka?',
    'Add more beach cities',
  ...
  const handleSend = (text?: string) => {
    ...
    } else if (userText.includes('pasikuda') || userText.includes('beach')) {
      response = '📍 Pasikuda Beach, Kalkudah, Coral reefs, snorkeling spots 🐠🌊';
    } else if (userText.includes('remove flights')) {
      response = '✈️ Switched to scenic coastal routes 🚆';
    } else if (userText.includes('add cities')) {
      response = '🌆 Added Trincomalee + Arugam Bay + Pasikuda!';
    } else {
      response = '🤖 I can help with budget, beaches & itinerary!';
    }
        ...
        <div className="flex-1 p-8 space-y-8 overflow-y-auto">
          ...
          >
            <div className="relative w-52 h-36 rounded-2xl overflow-hidden">
              <Image src="/pasikuda.jpg" alt="Pasikuda Beach" fill className="object-cover" />
            </div>

            <div className="flex-1">
              <h1 className="text-3xl font-bold text-gray-800">Sri Lanka Beach Escape 🇱🇰</h1>
          ...
          >
            <h2 className="text-2xl font-bold text-gray-800">🏖 Pasikuda Beach</h2>

            <p className="text-gray-600 mt-2 text-sm">
              One of Sri Lanka’s calmest beaches with crystal-clear shallow waters and coral reefs.
            </p>

            <iframe
              src="https://www.google.com/maps?q=Pasikuda,Sri%20Lanka&output=embed"
            ...
            <div className="mt-6 relative overflow-hidden rounded-3xl p-6 bg-gradient-to-br from-indigo-50 via-white to-blue-50 border border-white/50 shadow-lg">
              <div className="flex items-center gap-3 mb-3 relative z-10">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800">Why visit Pasikuda?</h3>
                  <p className="text-xs text-gray-500">Sri Lanka’s Calm Blue Paradise</p>
                </div>
              </div>

              <p className="text-sm text-gray-700 leading-relaxed relative z-10">
                <span className="font-semibold text-indigo-700">Pasikuda</span> is famous for its long shallow
                coastline, turquoise water, and peaceful tropical escape perfect for relaxation 🌊
              </p>
              ...
              <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3 relative z-10">
                <div className="flex items-start gap-2 bg-white/60 p-3 rounded-xl shadow-sm hover:shadow-md transition">
                  ...
                  <p className="text-sm text-gray-700">Shallow crystal-clear lagoon waters</p>
                </div>

                <div className="flex items-start gap-2 bg-white/60 p-3 rounded-xl shadow-sm hover:shadow-md transition">
                  <span className="text-lg">🐠</span>
                  <p className="text-sm text-gray-700">Snorkeling & coral reef exploration</p>
                </div>

                <div className="flex items-start gap-2 bg-white/60 p-3 rounded-xl shadow-sm hover:shadow-md transition">
                  <span className="text-lg">🏝</span>
                  <p className="text-sm text-gray-700">Peaceful uncrowded beach experience</p>
                </div>

                <div className="flex items-start gap-2 bg-white/60 p-3 rounded-xl shadow-sm hover:shadow-md transition">
                  <span className="text-lg">🌅</span>
                  <p className="text-sm text-gray-700">Stunning sunrise over the ocean</p>
                </div>
              </div>
